...
class DetailPanel {
  ...
  _getCommandsForNode(node) {
    ...
    if (node.nodeType === 'feature') {
      ...
      commands.push({ label: 'Copy: /edit-section er_diagram', action: 'copy', command: '/edit-section er_diagram' });
    }
    return commands;
  }
}
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  document.getElementById('btn-layout-toggle').addEventListener('click', () => {
    ...
    document.getElementById('layout-dir-label').textContent = app.state.layoutDirection === 'FISH' ? 'Fish' : 'Tree';
  });
