...
- **Identity semantics vs OKF (v3.4.3 — interop footgun to know):** OKF's *Concept ID* = the file **path** (minus `.md`), so an OKF consumer keys a concept by its path; brain keys by **`title`** (upsert-by-title, GLOBAL scope §2). Consequence for cross-system round-trips: brain joins on title, not path — **renaming a file (path) but keeping the title still resolves to the same note; changing the title at the same path targets/creates a *different* note** (the reverse of a path-keyed tool's intuition). When exchanging bundles with a path-keyed OKF tool, keep **titles** stable as the join key; treat slug/path as display-only.
...
**OKF conventional body headings (SPEC.md §5/§8 — `# Schema`, `# Examples`, `# Citations`) — deliberately not synthesized (v3.4.3):** these headings are *optional* conventions; brain keeps a note's body structure as-is rather than reshaping it. Provenance is carried as the `Source: <pointer>` line (round-trippable via §1 item 7 dual-write) — this **fills the role of OKF `# Citations`** but stays a **line, not a heading**, so import can read it back (a `# Citations` heading would break the `Source:` readback). Structured schema stays inline in the note body (brain does not emit a `# Schema` block). An OKF consumer that looks for `# Citations` still finds the source in the body prose; nothing is lost, only rendered brain-idiomatically.
...
### 8.7 log.md — Change History (v3.4.2; opt-in since v3.4.4)
...
⚠️ **Opt-in via `--log` (default OFF) — verified against the reference consumer (2026-07-14):** running Google's `reference_agent` `generate_visualization` on a real bundle shows its `_walk_concepts` reserves **only `index.md`, not `log.md`** (contrary to SPEC.md §7). An emitted `log.md` is therefore picked up as a spurious concept node (`type: Unknown`, title `log`) linked to *every* note it lists — in a real bundle that is a hub connected to all N notes, wrecking the force-directed graph layout. So brain emits `log.md` **only with the `--log` flag**; default bundles stay clean for the reference visualizer. `log.md` is optional in the spec, so omitting it is fully conformant.

**Export (graph → log.md) — only when `--log` is passed; one root `log.md` per bundle, body-only:**
...
- **`--history-detail` (implies `--log`; costs N extra calls):** for each note also call `mcp__graph-brain__get-note-history` → emit one `**Update**` entry per version with its changelog reason; **warn about the extra call/token cost first** (same threshold as the >100 notes warning). Not default because it doubles the call count.
