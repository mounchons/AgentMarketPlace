...
| `/brain-export [project\|--all-projects]` | v3.4: export ความรู้เป็น [OKF bundle](https://cloud.google.com/blog/products/data-analytics/how-the-open-knowledge-format-can-improve-data-sharing) — portable markdown + frontmatter ลง git/แชร์ได้โดยไม่ต้องมี MCP, เปิดกับ OKF visualizer ได้ตามสเปก (read-only, กติกาใน GRAPH_PROTOCOL §8) |
...
- 📤 **`/brain-export`** — dump per-project เป็น OKF bundle: 1 note = 1 ไฟล์ markdown + YAML frontmatter, LINKS_TO → relative links, MOC → `index.md`, ลง git/แชร์ข้ามทีมได้โดยไม่ต้องมี MCP (เปิดกับ static visualizer ของ OKF ได้ตามสเปก v0.1 — ยังไม่ได้ทดสอบกับ visualizer จริง); catalog-first + pre-write secret check (§6.3) + structural validation; read-only ต่อ graph
