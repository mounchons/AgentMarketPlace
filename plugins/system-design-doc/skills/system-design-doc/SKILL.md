...
|---------|------|---------|
| 2.2.0 | 2026-06-13 | Docs-sync release: README + `/help` refreshed to all 22 commands; main entry `/system-design-doc` + `/reverse-engineer` + `/import-plan` migrated to split-layout default (no single-file contradiction); frontmatter added to `/sync-with-features`, `/sync-with-mockups`, `/validate-integration`; allowed-tools fixes (`/brainstorm-design` +Write/Bash, `/sitemap-scan` Agent→Task); `${CLAUDE_PLUGIN_ROOT}` on all intra-plugin paths; `/create-diagram` Architecture type (8 total); cross-plugin contract fix with qa-ui-test v2.6.1 (`/qa-trace` registry-first + flat `AC-NNN` + `scenario.use_case_id`). No SKILL rule/command changes. |
