...
name: long-running
version: 2.1.0
description: Harness สำหรับ AI Agent ทำงานข้าม context windows รองรับ multi-session, feature tracking, progress logging, incremental development, epic grouping, subtask tracking, acceptance criteria, model assignment, review system, flows, state contracts, component requirements และ integration กับ ui-mockup, system-design-doc, dotnet-dev skills
---

# Long-Running Agent Skill

> **Version 2.1.0** - เพิ่ม Model Assignment & Review System (opus architect + implementer models, /review command, pattern adherence tracking)
...
| **Review งาน model อื่น** | `/review` หรือ `/review #6` |
...
- opus reviews งานของ model อื่น เทียบกับ reference implementation
- **Hybrid strategy**: Critical/High issues → opus แก้เองทันที, Medium/Low → ส่งกลับ model เดิม
- Review result: `pass` (80-100), `pass_with_suggestions` (60-79), `fail` (0-59)
- Failed features กลับเป็น `in_progress` พร้อม `blocked_reason`
- Auto-fixed issues มี commit hash ติดตามได้
- `/review` command — auto-select หรือระบุ feature ID

### Review Fix Flow (เมื่อ model ถูกส่งกลับ)
```
/continue → พบ feature ที่ fail review → อ่าน issues → อ่าน reference →
แก้ไข → commit review-fix(#X) → mark passed → แจ้ง user รัน /review อีกครั้ง
```
- `/continue` ตรวจจับ features ที่ `status == "in_progress"` + `review.result == "fail"` อัตโนมัติ
- Review fix มีความสำคัญสูงกว่า feature ใหม่ — ต้องแก้ก่อน
- หลังแก้ → ล้าง `review: null` → รอ `/review` รอบใหม่

### Feature Fields ใหม่ (v2.1.0)
| Field | Type | Description |
|-------|------|-------------|
| `assigned_model` | string \| null | model ที่รับผิดชอบ feature |
| `is_reference_impl` | boolean | true = ตัวอย่างที่ model อื่นต้องทำตาม |
| `review` | object \| null | ผลการ review โดย opus |

### Summary Fields ใหม่
| Field | Description |
|-------|-------------|
| `summary.model_workload` | จำนวนงานแยกตาม model |
| `summary.review_status` | สรุปผลการ review |
