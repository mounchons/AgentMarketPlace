...
1. **Read log file**
   ...
   - If file doesn't exist → show "ยังไม่มีประวัติ — ลอง /brain-scan-dev หรือ /brain-save-dev ก่อน"
...
2. **Apply filters based on arguments**
   ...
   Filters can be combined: `/brain-log-dev today --command scan`
...
5. **Suggest next actions**
   - ถ้าไม่เคย scan → แนะนำ `/brain-scan-dev`
   - ถ้า scan นานแล้ว (>7 days) → แนะนำ `/brain-scan-dev` (incremental)
   - ถ้ามีแต่ scan ไม่มี save → แนะนำ `/brain-save-dev`
