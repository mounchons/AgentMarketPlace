...
4. **Display results (Thai)**
   ...
   - If nothing found → suggest `/brain-scan-dev` or try different keywords

5. **Offer next actions**
   - "พิมพ์เลขเพื่อดูรายละเอียด หรือ:"
   - `/brain-explore-dev {title}` — เดินตาม graph ดู connections
   - `/brain-history-dev {title}` — ดูประวัติเปลี่ยนแปลง
   - `/brain-explain-dev {title}` — อธิบายแบบละเอียด
